...
spark = SparkSession.builder.appName("pyJoin").getOrCreate()
...
redisServerStreamingDF = redisServerStreamingDF.selectExpr("cast(key as string) key", "cast(value as string) value")
...
zSetEntriesDecodedStreamingDF.withColumn("customer", from_json("customer", customerJSONSchema))\
    .select(col("customer.*"))\
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
stediEventsStreamingDF = stediEventsStreamingDF.selectExpr("cast(key as string) key", "cast(value as string) value")
...
customerStreamingDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr("""email=customer
"""))
...
customerStreamingDF\
    ...
    .awaitTermination()
"""
customerStreamingDF.selectExpr("CAST(customer AS STRING) AS key", "to_json(struct(*)) AS value")\
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092")\
